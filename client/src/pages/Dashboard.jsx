...
export default function Dashboard() {
    ...
    const handlePayment = async () => {
        setLoading(true);
        try {
            const res = await axios.post(`${backend_url}/payment/create-order`, {
                token,
            });
            ...
            {user && (
                <div className="bg-white p-4 rounded shadow mb-6 text-sm md:text-base">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-2">
                        <p>
                            <b>Flat No:</b> {user.flatNo}
                        </p>
                        <p>
                            <b>Consumer ID:</b> {user.consumerId}
                        </p>
                        <p>
                        ...
                        <p>
                            <b>Monthly Fee:</b> ₹{user.maintenanceAmount}
                        </p>
                        <p>
                            <b>Last Payment:</b>{" "}
                            {user.lastPaidAt
